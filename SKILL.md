...
**日期段：不要从表格 B 列（日期）推。** 两者经常不一致，实测：
第 88 行 B=9月8日、编号却是 `26.8.31-MJ-88-1`；第 91 行 B=9月9日、编号是 `26.9.10-FF-91-1`。

正确做法是**从同一批已填过的行里学**——同一批（相邻、同需求创建人、同业务）用同一个日期段。
本次 182-191 用的 `26.9.14` 就是从旁边已填的 172-181 学来的。
**同一批一条都没填过时，问用户**，别拿 B 列凑。
