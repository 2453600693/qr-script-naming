...
【原片路径】
（粘贴文件夹路径，多个一行一个）
※ 文件夹名必须写成模特名，且跟素材总表里的模特名一字不差
   例：D:\原片素材\9.14\梁思思    D:\原片素材\9.14\杜少阳
   表里没有的模特名，我会自动加进模特列的下拉列表
...
**【原片路径】那行的提醒不能省**：素材必须按模特分文件夹，文件夹名就是模特名。
这条不写清楚，用户随手把视频堆在一个大目录里，模特列就填不出来。
...
**追加到表尾，不要往已有内容的中间插行**（往中间插会串号，这是硬禁区）。

- 先探清表格最后一个有数据的行（`sheet_probe.py`），从它后面一格开始写
- 新行除了脚本内容、原片编号，其余能推的列也要补齐（需求创建人／日期／平台／业务／摄像师／模特／剪辑师，来源见下一节）
- 「不能插行」专指别插到中间；追加到末尾是允许的
...
编号 `26.9.14-FF-91-1` 的四段各有来源，其中两段**猜错就会全批填错**：

**日期段 = 处理当天。** 哪天让本工具处理这批素材，全部编号就用哪天的日期：
9 月 14 日处理 → 全部是 `26.9.14-...`。格式 `年.月.日`，**不补零**（`26.9.8`、`26.8.31` 都合法）。

表格里的两个日期列都别拿来当编号日期：

- **B 列是「需求创建日期」**，与编号日期无关（实测第 88 行 B=9月8日、编号却是 `26.8.31`）
- **K 列是「成片日期」**，剪辑完成后才填

**拍摄人段：用实际拍摄人，不是需求创建人。** 实测：第 91 行 A=志良（运营提的需求），
编号却是 `26.9.10-FF-91-1`。实际拍摄人从脚本文档的章节判断：脚本在 MJ 节 → MJ，在 FF 节 → FF。

**序号段**：表格行号。**尾数段**：该脚本的原片总数。
...
模特列通常是固定列表下拉（`type=list`）。拿目录名当模特名写进去之前，
**先确认这个名字在选项里**，否则会落成无效值。缺了就用工具补：

```bash
python "$SK/ensure_dropdown.py" --url "<表格URL>" --sheet-id Amvc8S --column H \
    --names "梁思思,杜少阳" --range-rows H2:H446
```

- 缺失时自动追加，**原有选项一个不减**（写回的是"原有 + 新增"整体）
- `--dry-run` 先看缺哪些，不写入
- 写完回读校验，若发现原有选项丢失会直接非零退出
- 下拉类型不是 `list`（如 `listFromRange` 引用区域）时脚本会停下，让人确认怎么加
