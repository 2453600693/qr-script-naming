...
**模特靠目录结构**：约定「一个目录 = 一个模特」，目录名就是模特名（如 `D:\...\9.14\梁思思`）。
脚本只扫一层，别给上层目录指望递归。目录名不是模特名时（如 `律师10`），要问用户。

**写模特名前先确保它在下拉列表里**：模特列通常是固定列表下拉（`type=list`）。
目录名不在选项里时，先补进去再写值：
...
原有选项一个不减；`--dry-run` 可先看缺哪些；下拉不是 `list` 类型时脚本会停下让人确认。
