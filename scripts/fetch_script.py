...
# 条目标题：01劳纠 / 43团购 / 逾期1
ITEM = re.compile(r"^(\d{2})(劳纠|团购)$|^(逾期\d+)$")
# 需要丢掉的拍摄提示行
    ...
    for m in TOK.finditer(content):
        if m.group(1):
            ...
            if lvl == "h2":
                ...
                if hit:
                    code = title
                    cur = {"section": section, "code": code,
                           "type": hit.group(2) or "加拍",
                           "no": int(hit.group(1)) if hit.group(1) else None,
                           "lines": []}
                    items.append(cur)
                    paras = cur["lines"]
        else:
...
def main():
    ...
    items = parse(fetch(args.url, args.cache))
